...
`.trim(),
  ...
  renderUI: {
    name: "renderUI",
    description: `
Render an ephemeral interface in the chat. Pass exactly one JSX catalog element and an optional JSON \`state\` object. JSX is parsed as data and never executed. Use only this strict catalog: Stack(gap); Row(gap,align,justify,wrap); Card(title,subtitle/description); Text(label,tone,size,strong,mono); Heading(label,level); Badge(label,tone); Divider(); Button(action,label,variant,disabled); Input(value,label,placeholder,description/hint,type); Select(value,options,label,placeholder); Checkbox(checked,label,description); Slider(value,min,max,step,label,valueLabel); Table(columns,rows/data/items); ProgressBar(value,max,label,valueLabel,showValue); Callout(label,title,tone); KeyValue(items/entries/pairs). Image and HTML tags are unavailable. Variant values are case-insensitive and numeric props may be numeric strings. Select options are literal strings or \`{value,label}\`; Table columns are literal strings or \`{key,label,align}\`; KeyValue items are literal \`{label,value}\` objects. Only Button.action is required.

For interactive values use \`bind("path.to.value")\`, where the path names a primitive leaf supplied in \`state\` (numeric segments address arrays). Bind is allowed only on Input/Select \`value\`, Checkbox \`checked\`, and Slider \`value\`. Buttons require a stable non-empty \`action\` string and submit the whole state once. Example: \`<Stack gap="md"><Input label="Name" value={bind("form.name")} /><Button action="submit">Submit</Button></Stack>\` with \`state: {form:{name:""}}\`. Braced prop values may contain only string/number/boolean/null literals, literal arrays/objects, or that exact bind call. Identifiers, other calls, template literals, imports, functions, operators, spreads, conditionals, loops, handlers, and dangerouslySetInnerHTML are rejected structurally. Prefer KeyValue over a two-column Table, use at most one primary Button and one Callout, and avoid repeating prose directly above the card.
`.trim(),
    parameters: Type.Object({
      jsx: Type.String({
        description: "One literal-only JSX element against the renderUI catalog; source is parsed, never executed.",
      }),
